...
class Schedule {
    ...
    private var dayOfWeek = Days.DEFAULT
    ...
    operator fun String.rangeTo(other: String): Pair<LocalTime, LocalTime> =
        LocalTime.parse(this, timeFormatter) to LocalTime.parse(other, timeFormatter)

    infix fun Pair<LocalTime, LocalTime>.schedule(lessons: String) {
        val entity = ScheduleEntity(lessons, first, second)
        dayOfWeek.takeIf { it != Days.DEFAULT }?.let {
    ...
    private fun handleDay(day: Days, function: Schedule.() -> Unit) {
        dayOfWeek = day
        function()
        dayOfWeek = Days.DEFAULT
    }
}
